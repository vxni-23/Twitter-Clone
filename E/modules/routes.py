...
def message(user1, user2):
    ...
    for i in sender.conversation:
        if i.receiver == receiver_id:
            packet = [i.id, i.message, sender.username, i.message_stamp, i.sender]
            to_be_displayed.append(packet)

    for i in receiver.conversation:
        if i.receiver == sender_id:
            packet = [i.id, i.message, receiver.username, i.message_stamp, i.sender]
            to_be_displayed.append(packet)

    to_be_displayed.sort(key=lambda x:x[0])

    return render_template('chat.html', sender_id = sender_id, receiver_id = receiver_id, 
    sender = sender, receiver = receiver, user1 = user1, user2 = user2, history = to_be_displayed)